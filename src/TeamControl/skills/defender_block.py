...
from TeamControl.skills._shared import (
    GOAL_HW_M, DEF_BLOCK_ADVANCE_M, attack_sign, own_goal, angle_to,
)
...
def defender_block(snap: Snapshot, robot: RobotState | None, target) -> IntentMove | None:
    ...
    Finds the own teammate nearest our goal (assumed to be the goalie), then
    ...
    # Advance a little off the goal line, toward the field (opponent goal side).
    block_pos = (goal_x + attack_sign() * DEF_BLOCK_ADVANCE_M, block_y)
